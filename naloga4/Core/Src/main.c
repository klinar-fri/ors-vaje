...
int main(void) {
	...
	// myDelay --> isto kot HAL_Delay z getTick

	void MyDelay(uint32_t delay) {
		uint32_t zacetek = HAL_GetTick(); // tick ob zacetku

		while ((HAL_GetTick() - zacetek) < delay) {
	...
	volatile uint32_t zakasnitev = 3000;
    ...
    while (1){
        ...
        MyDelay(zakasnitev);
        stanjeGumba = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);
		...
		if(stanjeGumba){
			zakasnitev = zakasnitev - (zakasnitev / 3);
		}
